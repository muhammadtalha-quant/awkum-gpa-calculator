 ...
 */
export function calculateSGPA(data: { gradePoint: GradePoint; credits: Credit }[]): GradePoint {
 ...
 */
export function calculateCGPA(semesters: { sgpa: GradePoint; credits: Credit }[]): GradePoint {
...
export function distributeRetakeMarks(
  ...
  const currentTotalQP = semesters.reduce(
    (s, sem) => s + Number(sem.sgpa) * Number(sem.credits),
    0,
  );
  ...
  const achievedQP = eligible.reduce(
    (s, e) => s + (requiredGPs.get(e.subjectId) ?? e.currentGP) * e.credits,
    0,
  );
