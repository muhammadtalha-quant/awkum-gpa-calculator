...
import { SGPASubject, CGPASemester, GradePoint, Credit, Mark, asMark, asCredit, asGP } from '../domain/types';
...
interface AcademicState {
    ...
    updateSemesterSubject: (semesterId: string, subjectId: string, updates: Partial<SGPASubject>) => void;
    addSemester: () => void;
...
export const useAcademicStore = create<AcademicState>((set) => ({
    ...
    updateSemesterSubject: (semesterId, subjectId, updates) => set((state) => {
        const nextSemesters = state.semesters.map(sem => {
            if (sem.id === semesterId) {
                ...
                const semGp = calculateSGPA(nextSubjects.map(s => ({ gradePoint: s.gradePoint, credits: s.credits })));
    ...
    removeSemester: (id) => set((state) => ({
        semesters: state.semesters.filter(s => s.id !== id).map((s, idx) => ({ ...s, name: `Semester ${idx + 1}` }))
    }))
}));
